...
public class TelaPrincipal extends JFrame {
    ...
    //MUDAR CONSOANTE O NUMERO DE PARES QUE SE UTILIZA
    private int pares = 3;
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                for (ControleBotoesSelecionados cont : listaControle) {
                    if (cont.getReferenciaBotoes().get(botao) != null) {
                        ...
                        //adiciona o botao escolhido à lista de selecionados se ainda nao estiver
                        if (!listaSelecionados.contains(cont)) {
                            listaSelecionados.add(cont);
                        }
                        //numero de botoes selecionados
                        System.out.printf("lista selecionados: %d \n", listaSelecionados.size());
                        if (jogadas == QUANTIDADE_JOGADAS) {
                            ...
                            if (listaSelecionados.size() > 1) {
                             ...
                             // SE A LISTA DE SELECIONADOS == 1, OU SEJA, ENCONTROU-SE UM PAR
                            }else if(listaSelecionados.size()==1 ){
                                // MENOS UM PAR (É O MESMO DE TERES PARES-1)
                                pares--;
                                // SE O NUMERO DE PARES É 0 É PORQUE ENCONTROU TODOS OS PARES
                                if(pares == 0){
                                    for (ControleBotoesSelecionados cbs : listaSelecionados) {
                                        cbs.ParesTodosEncontrados();
                                    }
                                }
                            }
